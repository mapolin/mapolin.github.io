...
function drawThreeGeo(json, radius, shape, options) {
    ...
    for (var geom_num = 0; geom_num < json_geom.length; geom_num++) {
        ...
        var geometry = json_geom[geom_num].geometry;
        var properties = json_geom[geom_num].properties;

        if (geometry.type == 'Point') {
            convertCoordinates(geometry.coordinates, radius);            
            drawParticle(y_values[0], z_values[0], x_values[0], options, properties);
            
        } else if (geometry.type == 'MultiPoint') {
            for (var point_num = 0; point_num < geometry.coordinates.length; point_num++) {
                convertCoordinates(geometry.coordinates[point_num], radius);           
                drawParticle(y_values[0], z_values[0], x_values[0], options, properties);                
            }
            ...
            drawLine(y_values, z_values, x_values, options, properties);
            
        } else if (geometry.type == 'Polygon') {                        
            for (var segment_num = 0; segment_num < geometry.coordinates.length; segment_num++) {
                coordinate_array = createCoordinateArray(geometry.coordinates[segment_num]);           
            ...
            for (var polygon_num = 0; polygon_num < geometry.coordinates.length; polygon_num++) {
                for (var segment_num = 0; segment_num < geometry.coordinates[polygon_num].length; segment_num++) {
                    coordinate_array = createCoordinateArray(geometry.coordinates[polygon_num][segment_num]);           
...
function drawParticle(x, y, z, options, geom_data) {
...
function drawLine(x_values, y_values, z_values, options, geom_data) {
    ...
    // drawText(geom_data.name_long, line)
    
    clearArrays();
}

function drawText(text, location) {
    if(countries.indexOf(text) !== -1) return;
    ...
    var text3d = new THREE.PlaneGeometry(c.width, c.height);
    ...
    location.add(textMesh);

    location.geometry.computeBoundingBox();
    var center = location.geometry.boundingBox.center();

    textMesh.position.normalize().add(center).multiplyScalar(1.01)

    // textMesh.position.normalize().multiplyScalar(geomap.radius+10);
